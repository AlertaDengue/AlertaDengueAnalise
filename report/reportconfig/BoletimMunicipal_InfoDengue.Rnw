...
\input{report/reportconfig/structure_infodengue.tex} % Include the document which specifies all packages and structural customizations for this template
...
\begin{figure}[H]
\centering\includegraphics[width=1\linewidth]{report/reportconfig/InfoDengue2.png}  
\end{figure}
