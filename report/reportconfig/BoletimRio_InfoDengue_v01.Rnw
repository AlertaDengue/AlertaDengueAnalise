...
load('../RJ/figs/paramsRJ.RData') #parametros do estado e regionais 
load('../Rio_de_Janeiro/paramsRio.RData') # parametros do municipio

datarel = data_relatorio - ano*100
figmapaestado = paste("../",sigla,'/figs/','Mapa_E',sigla,'.png',sep='')
figmapamunicipio = paste("../",sigla,'/figs/','Mapa_E',sigla,'.png',sep='')
#figmunicipio = 'RJ/CamposdosGoytacazes/figuraCamposdosGoytacazes.png'
@

\fancyfoot[C]{Boletim Municipal - Rio de Janeiro}
\fancyfoot[R]{SE \Sexpr{(datarel)} de \Sexpr{ano}}
...
\textbf{Semana \Sexpr{(datarel)} de \Sexpr{ano}} % se, ano, em pp.RData
...
Desde o início do ano, \Sexpr{totano} casos foram registrados no estado, sendo \Sexpr{totultse} na semana \Sexpr{se}. A figura abaixo mostra as condições de transmissão em cada município.
...
i  = which(regs=="Metropolitana I")
   
      cat('\\begin{minipage}[t]{.66\\linewidth}\n')  # inicio da minipage
      # titulo:
      cat(paste('\\hypertarget{regional}{\\heading{Situação da Dengue na Regional ',regs[i],'}{6pt}}\n',sep='')); 
      ...
      mapareg=paste('/home/administrador/Repositorios/AlertaDengueAnalise/report/RJ/figs/Mapa',sigla,'_',nomesemacento,'.png',sep='')
      ...
      #             '.tex',sep='')
      tabreg=paste('/home/administrador/Repositorios/AlertaDengueAnalise/report/RJ/figs/',
      ...
      # Grafico regional:
      figname = paste('/home/administrador/Repositorios/AlertaDengueAnalise/report/RJ/figs/tweet',sigla,'_',nomesemacento,'.png',sep='')
...
\begin{center}
\input{../Rio_de_Janeiro/tabelaRio.tex}
\end{center}
...
coresbullets = c(verde='\\gsquare',amarelo='\\ysquare',
                            laranja='\\osquare',vermelho='\\rsquare')
for (j in 1:10) {
      nivel = res$nivel[res$aps == aps[j] & res$se==data_relatorio]
      corbullet = '\\wsquare'
      if(!is.na(nivel)) corbullet = coresbullets[nivel] 
...
aps <- unique(res$aps)
cat(paste('\\captionof{table}[tabregional]{Resumo das últimas seis semanas epidemiológicas nas Áreas Programáticas de Saúde}\n'))
cat('\\begin{center}\n')
for (i in 1:5){
      cat(paste('\\captionof{table}{APS',aps[i],'}\n'))
      tabaps=paste('../Rio_de_Janeiro/tabela',i,'.tex',sep='')
      cat(paste('\\input{',tabaps,'}\n',sep=''))
}
cat('\\end{center}\n')
cat('\\small{\\hyperlink{vartab}{ver descrição das variáveis}}\n')
