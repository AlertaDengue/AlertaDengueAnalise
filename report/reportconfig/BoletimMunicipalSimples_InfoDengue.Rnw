...
% tabela municipal
\captionof{table}[tabmun]{Resumo das últimas seis semanas epidemiológicas no município.}
\begin{center}
...
\item [pr(aumento)] probabilidade de aumento de incidência. Acima de \%90 é um indicador importante.
