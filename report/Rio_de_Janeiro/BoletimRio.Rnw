...
load('figs/paramsRio.RData') # parametros dos municipio
...
mapareg=paste('../RJ/figs/Mapa',sigla,'_',nomesemacento,'.png',sep='')
...
# Grafico regional:
figname = paste('../RJ/figs/tweet',sigla,'_',nomesemacento,'.png',sep='')
...
<<tabelas_saps,echo=FALSE,results=tex>>=
  aps <- unique(res$aps)
  cat(paste('\\begin{center}Resumo das últimas quatro semanas epidemiológicas nas Áreas Programáticas de Saúde}\n'))
  cat('\\begin{center}\n')
  for (i in 1:5){
    cat(paste('\\captionof{table}{APS',aps[i],'}\n'))
    tabaps=paste('figs/tabela',i,'.tex',sep='')
...
<<tabelas_saps,echo=FALSE,results=tex>>=
aps <- unique(res$aps)
cat(paste('\\begin{center}{Resumo das últimas quatro semanas epidemiológicas
