...
library("xtable")
load('../PR/Regionais/Cascavel/figs/paramsPR.RData') # parametros do estado e regionais
load('figs/paramsAnahy.RData') # parametros do municipio

datarel = data_relatorio - ano*100
figmapaestado = paste("../",sigla,'/figs/','Mapa_E',sigla,'.png',sep='')
figmaparegional = paste("figs/","Mapa_E",sigla,".png",sep="")
@

\fancyfoot[C]{Boletim 10ª Regional de Saúde - Cascavel}
...
\item \hyperlink{regional}{A RS de Cascavel}
\end{itemize}

\textbf{Os Municípios}

<<echo=FALSE,results=tex>>=
i  = which(regs=="Cascavel")
municip.reg <- municipios[municipios$nome_regional==regs[i],]

cat("\\begin{itemize}") #listar o nome dos municipios dentro da regional

for (i in 1:length(municip.reg)) {
      cat(paste("\\item \\hyperlink{",nickregs[[i]],"}{",municip.reg[i],"}\n",sep=""))
}
...
# O loop e'feito no R e o template em latex entra como string
      ...
      mapareg=paste("PR-RS-Cascavel/figs/Mapa",sigla,"_",nomesemacento,".png",sep="")
