...
for (i in 1:length(regs)){
      ...
      cat(paste("\\includegraphics[width=1.3\\textwidth]{",figname,"}",sep=""))
      cat("\\end{center}\n")
      ...
      cat("\\textbf{Municipios}") # titulo
       cat("\\begin{itemize}")
        ...
        coresbullets = c(verde="\\gsquare",amarelo="\\ysquare",
                            laranja="\\osquare",vermelho="\\rgsquare")
...
<<echo=FALSE>>=
    tabelaoref=paste(sigla,"/figs/tabelao",sigla,".tex",sep="")
@
