...
library("xtable")
direct = '~/Repositorios/AlertaDengueAnalise/report/RJ/'
source('~/Repositorios/AlertaDengueAnalise/report/RJ/configRelatorio.r')

figmapaestado = paste("RJ/figs/","Mapa_E",sigla,".png",sep="")
@
...
%-------------------------------------------------------------------------------
\hypertarget{titulo}{\heading{Situação da Dengue no Estado do \Sexpr{estado}}{6pt}} % \hypertarget provides a label to reference using 
...
for (i in 1:length(regs)){
      ...
      # titulo:
      cat(paste("\\hypertarget{",nickregs[i],"}{\\heading{Regional ",regs[i],"}}\n",sep="")) 
      
      # Mapa:
      mapareg = paste("RJ/figs/","mapa",sigla,"_",regs[i],".png",sep="")

      cat(paste("\\includegraphics[width=0.8\\textwidth]{",mapareg,"}",sep=""))

      # Two-column layout inicio
#      cat("\\begin{multicols}{2}\n")
      cat("\\vspace{0.5cm}")
      
#cat("Desde o início do ano,x casos foram registrados no estado, sendo y na última semana. A figura abaixo mostra as condições de transmissão em cada município.")
# Tabela 1  
      cat("\\captionof{table}{Resumo das últimas seis semanas epidemiológicas.}")
      cat("\\begin{center}\n")
      
      xtab<-xtable(ta,label="Table1")
      print(xtab, include.rownames=FALSE,floating=FALSE)
      cat("\\end{center}\n")

     # Tabela 2
     # cat("\\textbf{Tabela 2} Variação dos níveis de alerta observados durante as últimas seis semanas epidemiológicas.\n")
      
      # Two-column layout fim
 #     cat("\\end{multicols}\n")

      # Grafico:
      #cat("\\includegraphics[width=0.9\\textwidth]{Areal.png}\n")
      ...
      cat("\\textbf{Municipios}") # titulo
      cat("\\begin{itemize}")
      for (j in 1:length(municip)) {
            cat(paste("\\item \\hyperlink{",nickmunicip[j],"}{",municip[j],"}\n",sep=""))
      }
      cat("\\end{itemize}")
