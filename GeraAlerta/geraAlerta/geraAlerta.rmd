...
d2<-merge(d2,d[,c("SE","APS","data","tweets","estacao","casos","temp.min")],by=c("SE","APS"))
...
plot.alerta2<-function(ap){
      ...
      les = dim(d2v)[1]
      plot(1:les,d2v$casos,type="l",axes=FALSE,ylab="casos",xlab="")
      abline(v=which(d2v$alertaCli<3 & d2v$alertaRtweet<3 & d2v$alertaRt<3 & d2v$alertaCasos==0),
             col="darkgreen",lwd=1)
      abline(v=which(d2v$alertaCli>=3 | d2v$alertaRtweet>=3),col="yellow",lwd=1)
      abline(v=which(d2v$alertaRt>=3),col="orange")
      lines(1:les,d2v$casos)
      abline(v=which(d2v$inc>=100),col="red",pch=16)
      title(as.character(ap),cex=1.4)
      axis(2)
      axis(1,at=rev(seq(les,1,by=-12)),labels=d2v$SE[rev(seq(les,1,by=-12))],las=2,cex=0.3)
...
tiff("alerta.tif",res=200,compression = "lzw",width = 15,height = 22,units="cm")
  layout(matrix(1:10,nrow = 5),widths = rep(lcm(7),2),heights = rep(lcm(4)))
