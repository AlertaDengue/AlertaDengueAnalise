...
dadosAPS<-"dados_limpos/dadosAPS_201435.csv" # tirar quando for rodar a partir do atualiza_alerta
...
```{r echo=FALSE}
tapply(d$temp.min[d$SE==max(d$SE)],d$APS[d$SE==max(d$SE)],sum)
```
...
# funcao que calcula o indicador Rt>pcrit (default pcrit=0.95, lag=3)  # serve tanto para Rt de tweet como para casos,
# acumulado ou nao
...
for(i in 1:10) d2$ac.twgreat1[d2$APS==listaAPS[i]] <-Rtgreat1(d2$ptw1[d2$APS==listaAPS[i]],pcrit=0.90,lag=3)
