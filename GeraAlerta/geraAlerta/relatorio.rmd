...
d2 <- read.csv(file="../alerta/alertaAPS_201447.csv")
```
...
dc <- aggregate(d2[,"tempmin"],by=list(SE=d2$SE),FUN=mean,na.rm=TRUE) #clima
names(dc)[2]<-"tempmin"
...
pander(tail(cidade,n=12),compact=TRUE,)
...
- tempmin: média das temperaturas mínimas da semana
...
par(mfrow=c(1,1))
plot.alerta2("AP1")
tab<-tail(d2[d2$APS=="AP1",c("SE","tempmin","casos_est","inc","alertaClima","alertaTweet","alertaTransmissao","alertaCasos","nivel")],n=8)
```
...
ap = "AP2.1"
plot.alerta2(ap)
tab<-tail(d2[d2$APS==ap,c("SE","tempmin","casos_est","inc","alertaClima","alertaTweet","alertaTransmissao","alertaCasos","nivel")],n=8)
```
