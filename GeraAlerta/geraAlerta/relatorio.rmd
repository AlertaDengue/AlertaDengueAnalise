...
plot.alerta2<-function(d2=d2,ap){
  ...
  cores <- c("#0D6B0D","#C8D20F","orange","red")
  dv2.ts <- ts(d2v$casos_est,start=c(2010,1),freq=52)
  tempo <- time(dv2.ts)
  plot(dv2.ts,ylab="Casos",xlab="Ano")
  for (i in 1:4) {
    onde <- which(d2v$cor==i) 
     if (length(onde))
          segments(tempo[onde],0,tempo[onde],(d2v[onde,"casos_est"]),col=cores[i],lwd=3)
  }
...
par(mfrow=c(1,1))
plot.alerta2(d2,"AP1")
tab<-tail(d2[d2$APS=="AP1",c("SE","tmin","casos","casos_est","inc","nivel")],n=3)
names(tab)<-c("SE","temp","casos not","casos est","inc","nivel")
rownames(tab) <- NULL
```
...
ap = "AP2.1"
plot.alerta2(d2,ap)
tab<-tail(d2[d2$APS==ap,c("SE","tmin","casos","casos_est","inc","nivel")],n=3)
names(tab)<-c("SE","temp","casos not","casos est","inc","nivel")
rownames(tab) <- NULL
```
