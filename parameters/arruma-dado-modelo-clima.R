...
calcRt <- function(arq){
  ...
  res$umid_max[res$umid_max > 100] <- 100
  res$umid_med[res$umid_med > 100] <- 100
  res$umid_min[res$umid_min > 100] <- 100
  
  res$temp_med[res$temp_med > 40] <- 40
  res$temp_min[res$temp_min > 40] <- 40
...
summary.trees <- function(arq){
  ...
  for(r in 1:length(regs)){
    reg_geocode <- res$regional_geocodigo[res$regional_nome == regs[r]][1]
